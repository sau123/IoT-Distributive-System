...
public class MongoServiceClient {
	...
	public static String observe() {
		try{
			...
			DB database = client.getDB("cmpe273");
			DBCollection collection = database.getCollection("ClientResourceFridge");		
			DBObject abc = collection.findOne();	
			BasicDBObject query1 = (BasicDBObject)abc;	
			BasicDBObject doc1 = new BasicDBObject("$set" , new BasicDBObject().append("Mode", "observeMode"));
			collection.update(query1, doc1);
			MyThreadRunnable t = new MyThreadRunnable();
			Thread k = new Thread(t);
			k.start();
			
			return "Being observed !!";
